...
| HTTP 상태 코드                   | 설명                                | 해결 방법                   |
|:-----------------------------|:----------------------------------|:------------------------|
| **400 Bad Request**          | 멱등키의 길이가 300자를 초과함                | 멱등키의 길이를 확인하고 수정        |
| **409 Conflict**             | 동일한 키로 처리 중인 요청이 이미 존재함           | 잠시 후 재시도                |
| **422 Unprocessable Entity** | 동일한 멱등키를 사용했지만 요청 본문(Payload)이 다름 | 새로운 멱등키를 사용하거나 요청 본문 확인 |
