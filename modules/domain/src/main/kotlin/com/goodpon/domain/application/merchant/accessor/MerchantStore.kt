...
class MerchantStore(
    ...
    @Transactional
    fun createMerchant(merchantName: String, account: Account): Pair<Merchant, com.goodpon.domain.merchant.MerchantAccount> {
        val merchant = Merchant.create(name = merchantName)
        val savedMerchant = merchantRepository.save(merchant)

        val merchantAccount = com.goodpon.domain.merchant.MerchantAccount.createOwner(merchantId = savedMerchant.id, accountId = account.id)
